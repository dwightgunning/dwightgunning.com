...
function search(input, dims) {
  ...
  var regex =
    /^[\s\,]*([NSEW])?\s*([\-|\—|\―]?[0-9.]+)[°º˚]?\s*(?:([0-9.]+)['’′‘]\s*)?(?:([0-9.]+)(?:''|"|”|″)\s*)?([NSEW])?/;
...
function extractProperties(node) {
  ...
  if (links.length) {
    properties.links = links.map((link) =>
      Object.assign({ href: link.getAttribute('href') }, getMulti(link, ['text', 'type']))
    );
  }
  return properties;
}
...
const GEO_TYPES = ['Polygon', 'LineString', 'Point', 'Track', 'gx:Track'];
...
class Converter {
  ...
  async loadTopoJson() {
    ...
    if (topoJsonData.type === 'Topology' && topoJsonData.objects !== undefined) {
      result = {
        type: 'FeatureCollection',
        features: (result.features = Object.keys(topoJsonData.objects)
          .map((key) => topojsonFeature(topoJsonData, key))
          .reduce((a, v) => [...a, ...v.features], [])),
      };
    }
    return result;
  }
...
class Actor {
  ...
  onLoad() {
    return new Promise((res) => {
      this._ === undefined ? this.handlers.set(this.initId, { res: res, rej: res }) : res(this._);
    });
  }
  exec(command) {
    const that = this;
